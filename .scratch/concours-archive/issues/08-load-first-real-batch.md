...
- **Exercises tagged — Tunisia 2023 & 2024 (4 papers, 13 exercises).**
  ...
  - **De-imposed (done).** These four source PDFs were **2-up A3 booklet impositions** (PDF1 = [logical p4 | p1], PDF2 = [p2 | p3], PDF3 = [p5 | blank]). Rebuilt into clean sequential A4 PDFs: rendered each A3 page to PNG (pdftoppm 150 dpi), split at the midline and reordered p1..p5 with Pillow (p5 = whole PDF3 page trimmed to content, since it spills past the midline), re-saved as image PDFs (`scratchpad/deimp/deimpose.py`). Verified page-by-page (title on p1, parts in order, p5 complete). Anchors updated to the now-sequential logical pages. Text layer is gone (was garbage OCR anyway); these remain image PDFs but are now readable front-to-back.
